...
def apply_lib(doc, funcs, app_dir, objs):
   for k, v in funcs.items():
      if not isinstance(v, six.string_types):
         continue
      logger.debug(f"process function: {k}")
      old_v = v
      try:
...
def run_code_macros(f_string, app_dir):
   def rreq(mo):
      ...
      for filename in glob.iglob(path):   
         logger.debug(f"process code macro: {filename}")
         try:
...
def run_json_macros(doc, f_string, app_dir):
   ...
   def rjson(mo):
      if mo.group(2).startswith('_attachments'): 
         ...
         filenum = 0
         for filename in glob.iglob(path):
            logger.debug(f"process json macro: {filename}")
            library = ''
            try:
         ...
         logger.debug(f"process json macro: {mo.group(2)}")
         ...
         for i, field in enumerate(fields):
            if field not in library:
               logger.warning(f"process json macro: unknown json source: {mo.group(2)}")
               break
   ...
   for k, v in six.iteritems(included):
      varstrings.append(f"var {k} = {json.dumps(v).encode('utf-8')};")
